/**
 * Validates a scan target to prevent command injection and malformed requests.
 * Supports IPv4, IPv6, Domain Names, and Usernames.
 */
function isValidTarget(target) {
    if (!target || typeof target !== 'string' || target.length > 255) return false;

    // 1. Check if it's a valid IP (v4 or v6)
    ...
    // 2. Check if it's a valid Domain Name
    const domainRegex = /^(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z0-9][a-z0-9-]{0,61}[a-z0-9]$/i;
    
    // 3. Check if it's a valid Username (for Sherlock/Grim)
    const usernameRegex = /^[a-zA-Z0-9_-]{1,64}$/;

    // 4. Check for shell patterns (Safety net)
    const shellPattern = /[;&|><$`\\]/;
    if (shellPattern.test(target)) return false;

    return ipRegex.test(target) || domainRegex.test(target) || usernameRegex.test(target);
}
